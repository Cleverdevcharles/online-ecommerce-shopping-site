  ...
  const handleSubmit = (e) => {
    ...
    createProduct(user._id, token, formData).then((data) => {
      if (data.error) {
        ...
        setTimeout(() => {
          setLoading(false);
          // toast.error(`Product should be unique. ${name} already exist.`);
        }, 1000);
        console.log(data);
      } else {
        setLoading(true);
  ...
  const newPostForm = () => (
    <form className="mb-3" onSubmit={handleSubmit}>
      ...
      {shipping == 0 ? (
        ...
        <div
          className="form-group"
          style={quantity < 1 ? { display: "none" } : { display: "block" }}
        >
...
export default Create__Product;
